...
const HOMEWORKS: Homework[] = [
  ...
  { id: 2, student: 'Анна Мельник', topic: 'Essay: Future of AI', subject: 'Англійська', deadline: '10 Жов, 2023', status: 'ПЕРЕВІРЕНО', avatarBg: 'bg-[#ebe3ff]' },
  ...
  { id: 4, student: 'Дмитро Іванов', topic: 'Київська Русь: аналіз джерел', subject: 'Математика', deadline: '09 Жов, 2023', status: 'ПЕРЕВІРЕНО', avatarBg: 'bg-[#e7eff9]' },
        ...
        <div className="flex items-start gap-8">
          ...
          <div className="flex-1 flex flex-col gap-4">
            ...
            <div className="bg-white rounded-2xl border border-[#dee1e6] overflow-hidden">
              ...
              {filtered.map((hw, i) => {
                const isSaved = saved.includes(hw.id);
                const status = isSaved ? 'ПЕРЕВІРЕНО' : hw.status;
                return (
                  <div key={hw.id}
                    onClick={() => status === 'НЕ ПЕРЕВІРЕНО' && setSelected(p => p?.id === hw.id ? null : hw)}
                    className={`grid grid-cols-[2fr_2.5fr_1fr_1fr] items-center px-6 py-4 transition-colors ${i < filtered.length - 1 ? 'border-b border-[#dee1e6]' : ''} ${selected?.id === hw.id ? 'bg-[#f0f7ff] border-l-4 border-l-[#1f8cf9]' : status === 'НЕ ПЕРЕВІРЕНО' ? 'cursor-pointer hover:bg-[#f8f9fb]' : ''}`}>
                    ...
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full font-inter font-bold text-[10px] w-fit ${status === 'ПЕРЕВІРЕНО' ? 'bg-[#e0faea] text-[#1a7bd9]' : 'bg-orange-100 text-orange-600'}`}>
                      {status}
                    </span>
                  </div>
                );
          ...
          {selected && (
            <aside className="w-[300px] flex-shrink-0 bg-white rounded-2xl shadow-[0px_25px_50px_-12px_#00000040] overflow-hidden sticky top-24 animate-fade-in">
              {/* Header */}
              <div className="flex items-center justify-between p-5 border-b border-[#dee1e6]">
                <div className="flex items-center gap-3">
                ...
                <button type="button" onClick={() => setSelected(null)} aria-label="Закрити" title="Закрити"
