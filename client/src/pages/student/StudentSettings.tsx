    ...
    <StudentLayout>
      <div className="max-w-[800px] mx-auto flex flex-col gap-8">
        <div>
          ...
          <p className="font-inter text-[#565d6d] text-lg mt-2">Керуйте вашою персональною інформацією та налаштуйте відображення аватару.</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#dee1e6] overflow-hidden">
          ...
          <div className="flex items-center gap-4 p-6 border-b border-[#dee1e6]">
            ...
            <div>
              ...
              <p className="font-inter text-[#565d6d] text-sm">Ці дані будуть доступні викладачам та адміністрації</p>
            </div>
          </div>
          ...
          <div className="p-6 flex flex-col gap-6">
            {/* Avatar */}
            <div>
              <p className="font-inter font-bold text-[#565d6d] text-xs tracking-[0.60px] uppercase mb-3">ФОТО ПРОФІЛЮ</p>
              <div className="flex items-center gap-6 p-5 bg-[#f8f9fb] rounded-2xl border border-[#dee1e6]">
                ...
                <div className="flex flex-col gap-2.5">
                  ...
                  <p className="font-inter text-[#9095a1] text-xs">Дозволені формати: JPG, PNG. Максимальний розмір файлу — 5MB.</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-5">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label htmlFor="st-name" className="font-inter font-bold text-slate-900 text-sm">ПІБ (Повне ім'я)</label>
                ...
                <div className="relative">
                  ...
                  <input id="st-name" type="text" value={form.fullName} onChange={set('fullName')}
                ...
                <p className="font-inter text-[#9095a1] text-xs mt-1">Вкажіть актуальне ім'я для заповнення дипломів</p>
              </div>

              <div>
                <label htmlFor="st-telegram" className="font-inter font-bold text-slate-900 text-sm block mb-2">Telegram Nickname</label>
                <div className="relative">
                  ...
                  <input id="st-telegram" type="text" value={form.telegram} onChange={set('telegram')}
                    placeholder="@alex_kovalenko"
                    className="w-full border border-[#dee1e6] rounded-xl pl-10 pr-4 py-3 font-inter text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#1f8cf9]" />
                </div>
                <p className="font-inter text-[#9095a1] text-xs mt-1">Використовується для зв'язку в чатах</p>
              </div>

              <div>
                <label htmlFor="st-phone" className="font-inter font-bold text-slate-900 text-sm block mb-2">Номер телефону</label>
                <div className="relative">
              ...
              <div>
                <label htmlFor="st-email" className="font-inter font-bold text-slate-900 text-sm block mb-2">Електронна пошта</label>
                <div className="relative">
            ...
            {saved && (
              <div className="flex items-center gap-2 p-3 bg-[#e0faea] rounded-xl">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#1a7bd9" strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>
                <p className="font-inter text-[#1a7bd9] text-sm font-medium">Зміни успішно збережено!</p>
              </div>
            )}

            <div className="flex items-center justify-between pt-4 border-t border-[#dee1e6]">
              <button type="button" onClick={() => setForm({ fullName: `${user?.firstName ?? ''} ${user?.lastName ?? ''}`.trim(), telegram: user?.nickname ?? '', phone: user?.phone ?? '', email: user?.email ?? '' })}
