...
class RegistrationRequestView(APIView):
    ...
    def post(self, request):
        ...
        return Response({'message': 'Готово! Ваша заявка успішно відправлена менеджеру.', 'id': reg_request.id}, status=status.HTTP_201_CREATED)
    ...
    def post(self, request, pk):
        ...
        try:
            with transaction.atomic():
                ...
                user = User.objects.create_user(
                    username=reg_request.email, email=reg_request.email, password=password,
                    first_name=first_name, last_name=last_name, phone=reg_request.phone,
                    nickname=reg_request.telegram_nickname, role_obj=role_obj, is_approved=True,
                )
        ...
        try:
            send_mail(
                ...
                message=f'Логін: {reg_request.email}\nПароль: {password}',
                from_email=settings.EMAIL_HOST_USER,
        ...
        return Response({'message': f'Акаунт для {reg_request.email} успішно створено.', 'user_id': user.id}, status=status.HTTP_201_CREATED)
...
class StudentBalanceView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({'message': 'Профіль учня не знайдено.'}, status=status.HTTP_404_NOT_FOUND)

        # Отримуємо активний пакет
